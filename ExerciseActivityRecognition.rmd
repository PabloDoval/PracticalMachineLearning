...
Data captured using accelerometers on the belt, forearm, arm, and dumbell of 6 participants performing the same exercise - *Unilateral Dumbbell Biceps Curl* - with five different execution qualities is available at the article referenced above, and will be used for this study. The data available is split in two data sets:
 ...
 - *Test Data:*
      ...
      The test data provided has a similar data structure, but does not include the field **classe**, which is the feature to predict.
...
cleanTestData <- testData[, names(cleanTrainData[, -53])]
testCleanColumns <- ncol(cleanTestData)
testCleanRows <- nrow(cleanTestData)
```
 
The resulting data set for training contains now **`r trainCleanColumns` columns**, while retaining **`r trainCleanRows` observations**, while the data set for testing is **`r testCleanColumns` wide** with **`r testCleanRows` observations**.
